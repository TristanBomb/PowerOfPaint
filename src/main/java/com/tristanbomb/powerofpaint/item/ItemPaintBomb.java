...
public class ItemPaintBomb extends Item
{
	...
	public void initModel() {
		ModelResourceLocation defModel = new ModelResourceLocation(getRegistryName(), "inventory");
		ModelResourceLocation[] modelList = new ModelResourceLocation[EnumPaintTypes.values().length];
		ModelLoader.setCustomModelResourceLocation(this, -1, defModel);
		...
		ModelLoader.setCustomMeshDefinition(this, stack -> {
			int meta = stack.getMetadata();
			if (meta >= 0 && meta < EnumPaintTypes.values().length) {
				return modelList[stack.getMetadata()];
			}
			
			else return defModel;
		});
	}
    ...
    {
        for (int i = 0; i < EnumPaintTypes.values().length; ++i)
        {
